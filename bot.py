...
def setup_bot_menu():
    telegram_api("setMyCommands", {"commands":[{"command":"settings","description":"Открыть настройки"},{"command":"status","description":"Статус бота"},{"command":"help","description":"Справка"}]})
...
def send_telegram_message_with_button(text, button_text, button_url):
    if not BOT_TOKEN or not CHAT_ID:
        print("BOT_TOKEN или CHAT_ID не заданы")
        return
    payload = {"chat_id":CHAT_ID,"text":text,"parse_mode":"HTML","reply_markup":{"inline_keyboard":[[{"text":button_text,"url":button_url.strip()}]]}}
...
def parse_freelancehunt():
    ...
    try:
        ...
        for entry in feed.entries:
            ...
            send_telegram_message_with_button(format_freelancehunt_message(title, summary, category, budget if budget else None), "🔗 Открыть проект", link)
    except Exception as e:
    ...
    try:
        for url in KABANCHIK_URLS:
            ...
            for a in soup.find_all("a", href=True):
                ...
                send_telegram_message_with_button(format_kabanchik_message(title, category, "Описание на сайте Kabanchik", None), "🔗 Открыть задачу", full_link)
    except Exception as e:
        ...
        try:
            ...
            for update in updates:
                ...
                if "message" in update:
                    ...
                    if text == "/start":
                        send_telegram_message("Привет! Нажми кнопку Menu возле поля ввода или используй команды:\n/settings\n/status\n/help")
                    elif text == "/settings":
                        config = ensure_config_exists()
                        send_telegram_message(get_settings_text(config), create_settings_keyboard(config))
                    elif text == "/help":
                        send_telegram_message("📚 <b>ДОСТУПНЫЕ КОМАНДЫ:</b>\n\n/settings - Открыть настройки\n/help - Эта справка\n/status - Статус бота")
                    elif text == "/status":
                        config = ensure_config_exists()
                        send_telegram_message(f"✅ <b>БОТ РАБОТАЕТ</b>\n\n💰 Бюджет: ${config['freelancehunt']['min_budget']}\n🔍 Ключевые слова: {', '.join(config['freelancehunt']['keywords'])}")
                    ...
                    if data == "open_settings":
                        ...
                        keyword_map = {"kw_video":"видео","kw_edit":"монтаж","kw_ai":"AI","kw_anim":"анимация","kw_ads":"реклама","kw_photo":"фото"}
                        ...
                        telegram_api("answerCallbackQuery", {"callback_query_id": callback_id, "text": "Меню закрыто"})
                        continue
        except Exception as e:
