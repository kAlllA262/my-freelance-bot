...
def parse_kabanchik():
    """Парсинг Kabanchik по ВАШИМ ссылкам (кабинет заказчика)"""
    global kabanchik_session, kabanchik_sent_tasks
    
    print("🔍 Начинаю парсинг Kabanchik по вашим ссылкам...")
    ...
    try:
        ...
        # Проходим по каждой вашей ссылке
        for url in KABANCHIK_URLS:
            try:
                # Определяем категорию из URL
                category = "Неизвестно"
                for cat_slug, cat_name in KABANCHIK_CATEGORY_NAMES.items():
                    if cat_slug in url:
                        category = cat_name
                        break
                ...
                # Ищем заказы (пробуем разные классы)
                ...
                print(f"   📊 В категории '{category}' найдено: {count} заказов")
